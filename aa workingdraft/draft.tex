...
\abstract{Suspended sediment collected by vegetation in marshes and wetlands contributes to vertical accretion, which can buffer against rising sea levels, allowing these ecosystems to persist. Effective capture efficiency (ECE), a parameter quantifying the fraction of incoming particles that are directly captured by submerged vegetation stems and leaves, is a key variable in determining the significance of direct interception in elevation-change models. The factors governing ECE are not yet thoroughly understood in transitionally turbulent flows. Using a recirculating flume containing an array with a variable count of artificial cylindrical collectors, we conducted a set of 12 experiments at three flow speeds and three collector densities (plus equivalent control experiments without vegetation), in addition to four experiments where biofilm was allowed to accumulate. We determined that ECE decreases with increasing collector Reynolds number (study range: 66 to 200; p < 0.05 for two of three treatments) and collector density (solid volume fraction: 0.22\% to 1.17\%; p < 0.05 for two of three treatments). Adding biofilm increased ECE in all cases, by a multiplicative factor ranging from 1.53 to 7.15 at different collector densities and biofilm growth durations. In some cases, biofilm had an impact on ECE that far outweighed impacts of variation in physical parameters (i.e., flow and canopy configuration). By combining our data with those of one similar study, we also present a preliminary model quantitatively assessing the effect of collector density on ECE.}
...
There is also evidence of a positive correlation between $d_c$ and $a$ for some aquatic plant species and communities \cite{mudd2010does}. Since there is convincing evidence that reduced collector density results in reduced total capture rate \cite{Fauria_2015}, efforts to minimize $d_c$ should only go as far as is possible without also reducing collector density. \textit{Spartina alterniflora} is an example of a plant that inhabits coastal estuaries and wetlands and that has relatively high average $a$ values (3-5 \SI{}{\metre\squared/\metre\cubed}) and relatively low average $d_c$ (3-5 mm) \cite{mudd2010does}, though of course habitats in different climates might be better matched to other species that fulfill those criteria.

\vspace{6pt} 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% optional
\supplementary{Raw data and computer code for this study are available publicly on our \href{https://github.com/wingfactor/wingenroth-et-al-2021}{repository}.}
