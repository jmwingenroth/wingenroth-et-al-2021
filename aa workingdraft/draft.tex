...
In order to estimate ECE, we need to know the collection rate for water that is being acted upon by the collectors (i.e., in the test section), which requires scaling up by the ratio between the test section volume (1.95 m $\times$ 0.6 m $\times$ 0.4 m = \SI{0.468}{m^3}) and the water volume of the flume's entire wetted floorplan under experimental conditions (depth = 0.4 m in test section). To calculate the latter, we drained the flume after an experiment while measuring flow rate at the drain outlet over time, performed polynomial regression (Figure \ref{fig:flumevol}), and calculated a definite integral (Table \ref{tbl:flumevol}) from when draining began to when the part of the flume involved in the experimental flows was empty ($V = \int_{t_{full}}^{t_{empty}}{-\frac{dV}{dt}dt}$). Because polynomials of second order and above fit well, and all yielded very similar volume estimates, we felt confident using \SI{2.43}{\metre\cubed} as the flume volume in our calculations.
...
To minimize the effect of changing re-suspension rates on our analyses, we sought to minimize the positive curvature in our residual plot without making substantial sacrifices in terms of model fit. We went about doing so by conducting models for an $n \times m$ grid with $n$ timesteps dropped from the beginning and $m$ from the end of all experimental runs in the dataset. We calculated a crude estimate of curvature using the second-order coefficient of a quadratic model of the residuals, and the mean and minimum R$^2$ of the set of 12 linear models used in analyses, for each point in the grid (Figure). 
