...
Because we assume the shape of the sediment profile follows the Rouse equation, which is time-invariant, proportionality between the average concentration and the concentration at a given height ($\phi_a$) can be assumed to remain constant: $\bar{\phi}=\frac{\phi_a}{C}$. The solution of the above differential equation, 
\begin{equation}
    \bar{\phi}(t) = \bar{\phi}(0)e^{-kt}\,,
    \label{eq:avgconc}
\end{equation} 
relates $\bar{\phi}$ to its starting value, thus the proportionality constant $C$ is eliminated when $\phi_a$ is substituted: 
\begin{equation}
...
\centering
\caption{Conceptual diagram of the laboratory flume. Labeled parts are: 1) pump, 2) magnetic flowmeter, 3) test section, and 4) honeycomb flow collimator. Arrows indicate direction of flow. Dimensions not to scale.}
\label{fig:floorplan}
\end{figure}
