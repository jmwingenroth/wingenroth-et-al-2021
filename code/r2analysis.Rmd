...
p1 <- r2final %>%
  ...
  ggplot(aes(x = as.factor(as.numeric(start)-1), y = as.factor(as.numeric(stop)-1), fill = mean_r2, label = round(mean_r2,3))) +
  geom_tile() +
  geom_text() +
  scale_fill_gradient2(midpoint = .5, limits = c(0,1)) +
  labs(y = "Timesteps Removed from End of Run", 
       x = "Timesteps Removed from Beginning of Run", 
       fill = expression(paste("Mean ",R^{2})))

p2 <- r2final %>%
  group_by(start, stop) %>%
  summarise(min_r2 = min(r2)) %>%
  ggplot(aes(x = as.factor(as.numeric(start)-1), y = as.factor(as.numeric(stop)-1), fill = min_r2, label = round(min_r2,3))) +
  geom_tile() +
  geom_text() +
  scale_fill_gradient2(midpoint = .5, limits = c(0,1)) +
  labs(y = "Timesteps Removed from End of Run", 
...
p3 <- bind_rows(curvval) %>%
  ggplot(aes(x = as.factor(as.numeric(start)-1), y = as.factor(as.numeric(stop)-1), fill = curv, label = signif(curv, digits = 2))) +
  geom_tile() +
  geom_text() +
  scale_fill_gradient2(low = scales::muted("red"), mid = "white", high = scales::muted("red")) +
    labs(y = "Timesteps Removed from End of Run", 
       x = "Timesteps Removed from Beginning of Run", 
       fill = expression(paste(italic(t^{2})," Coefficient")))

final_plot <- plot_grid(p3, p1, p2, labels = c("(a)","(b)","(c)"), scale = .9) 
