...
fitdata %>%
  ggplot(aes(x = Re, y = k_t, ymin = k_t - 1.96*dk_t, ymax = k_t + 1.96*dk_t, color = as.character(round(ad,4)))) +
...
final %>%
  ggplot(aes(x = Re, y = k_s, ymin = k_s - 1.96*dk_s, ymax = k_s + 1.96*dk_s, color = as.character(round(ad,4)))) +
  geom_line(lty = "dotted", size = 2, alpha = .3) + 
  geom_point(position = position_dodge(width = 10)) +
  geom_errorbar(width = 10, position = "dodge") +  
  scale_color_manual(values = c("black","blue","green","red")) +
...
final %>%
  filter(dowel_density != "0000") %>%
  ggplot(aes(x = Re, y = k_c, ymin = k_c - 1.96*dk_c, ymax = k_c + 1.96*dk_c, color = as.character(round(ad,4)))) +
  geom_line(lty = "dotted", size = 2, alpha = .3) + 
  geom_point(position = position_dodge(width = 10)) +
  geom_errorbar(width = 10, position = "dodge") +  
...
final %>%
  filter(dowel_density != "0000") %>%
  ggplot(aes(x = Re, y = eta, ymin = eta - 1.96*deta, ymax = eta + 1.96*deta, color = as.character(round(ad,4)))) +
  geom_line(lty = "dotted", size = 2, alpha = .3) + 
  geom_point(position = position_dodge(width = 10)) +
  geom_errorbar(width = 10, position = "dodge") +  scale_color_manual(values = c("blue","green","red")) +
  scale_y_continuous(labels = scales::percent) +
  labs(y = "Effective capture efficiency", x = expression(Re[c]), color = expression(italic(ad))) +
  theme_minimal()
```
