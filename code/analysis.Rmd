...
for (k in 1:length(unique(pumpfinal$date))) {
  ...
  resids[[k]] <- cbind(tempdata,resid = resid(r2mod[[k]]))
...
resids <- tibble(bind_rows(resids))
...
plotece1 <- final %>%
  ...
  labs(y = "ECE", x = expression(Re[c]), color = expression(phi[c])) +
  theme_bw() +
...
plotece2 <- final %>%
  ...
  labs(y = "ECE", color = expression(Re[c]), x = expression(phi[c])) +
  theme_bw() +
  theme(legend.position = "bottom")

plotece <- plot_grid(plotece1, plotece2, labels = c("(a)","(b)"), scale = .9, align = "lrtb")

save_plot("../pics/ece_plot.png",plotece, dpi = 600)
...
cplot <- c_over_phi %>%
  ggplot(aes(x = phi, y = C, color = factor(lab, levels = c("This study", "Fauria et al. (2015)")))) +
  geom_line(data = c_phi_curve, aes(x = phi, y = C), inherit.aes = FALSE) +
  geom_point(size = 2, pch = 4) +
  theme_bw() +
  labs(x = expression(phi[c]), color = "") +
  scale_color_manual(values = c("darkgreen","red")) +
  scale_x_continuous(labels = scales::percent) +
  annotate("polygon", x=c(.012,.012,.004,.004,.007), y= c(23,27,27,23,23), color = "black", fill = "gray 97") +
  annotate("text", .008, 25, parse = TRUE, label = "C == 0.00801*phi[c]^-1.41")
...
plotcomp <- final %>%
  ...
  geom_line(data = fauria_pred, aes(y = eta_est, x = Re, linetype = "Fauria et al. model"), inherit.aes = FALSE, show.legend = TRUE) +
  geom_point() +
